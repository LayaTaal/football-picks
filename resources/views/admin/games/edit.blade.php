                    ...
                    <form method="POST" action="/admin/games/{{ $game->id }}" enctype="multipart/form-data">
                        ...
                        <div class="mb-6">
                            ...
                            <select
                                ...
                                @foreach( $teams as $team )
                                    <option value="{{ $team->id }}" {{ old( 'away_team', $game->away_team ) === $team->id ? 'selected' : '' }}>{{ $team->name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-6">
                            ...
                            <select
                                ...
                                @foreach( $teams as $team )
                                    <option value="{{ $team->id }}" {{ old( 'home_team', $game->home_team ) === $team->id ? 'selected' : '' }}>{{ $team->name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-6">
                            ...
                            <select
                                ...
                                @foreach( $rounds as $round )
                                    <option value="{{ $round->id }}" {{ old( 'round_id', $game->round_id ) === $round->id || $round->id == request()->get( 'round' ) ? 'selected' : '' }}>{{ $round->title }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-6">
                            <x-button type="submit">Update Game</x-button>
                        </div>
                    </form>
                </div>
            </div>
